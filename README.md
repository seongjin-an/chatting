...
- 메시지 흐름 — WebSocket 수신부터 DB 저장, 다시 브라우저 전달까지 12단계
- 모듈 구성표 — 포트, 역할 한눈에
...
message-service  (Kafka consumer)
  ...
  │     outbox: status=PENDING, destinationTopic=message-fanout
  ▼
message-service  (OutboxEventPoller · 100ms 주기 · SKIP LOCKED)
  │  5. PENDING 이벤트 감지 → 채널 멤버 목록 조회
  │  6. Kafka 발행 → [message-fanout] topic
  │     payload: MessageFanoutPayload { messageId, channelId, recipientIds[], ... }
  ▼
fanout-delivery-service  (Kafka consumer)
  │  7. recipientIds 순회 → Redis 조회
  │     ws:user:{userId}         → connectionKey Set
  │     ws:connection:{key}      → ConnectionInfo JSON (instanceId 포함)
  │  8. stale connectionKey 자동 제거 (TTL 만료된 항목)
  │  9. 인스턴스별 그루핑 후 Kafka 발행
  │     → [connection-instance-{instanceId}] topic (인스턴스별)
  ▼
connection-service  (Kafka consumer, 자신의 instanceId 토픽만 소비)
  │  10. WebSocket 세션 레지스트리에서 userId로 세션 조회
  │  11. TextMessage 전송 (ContentMessage JSON)
  ▼
Browser
  12. 메시지 수신 → Zustand 스토어 업데이트 → UI 렌더링
```
...
| Redis | redis:7 | 6379 | JWT 블랙리스트, WebSocket 연결 라우팅, seq 생성 |
...
해결:
  1. message + outbox 레코드를 같은 트랜잭션으로 MySQL에 기록
  2. OutboxEventPoller(100ms, SKIP LOCKED)가 PENDING 이벤트 감지
  3. 채널 멤버 조회 → MessageFanoutPayload 구성
  4. Kafka [message-fanout] 발행 → status = PROCESSED
  5. OutboxEventCleaner가 1일 경과 PROCESSED 이벤트 정리

SKIP LOCKED: 다중 인스턴스 환경에서 동일 이벤트를 중복 처리하지 않도록
...
| `message-fanout` | message-service (OutboxPoller) | fanout-delivery-service | Outbox 이벤트 + 수신자 목록 |
| `connection-instance-{instanceId}` | fanout-delivery-service | connection-service (인스턴스별) | 처리 완료 메시지 → WS 전달 |
...
- **channel_member**: channel_id + user_id (복합 PK), joined_at
...
│           ├── OutboxEventPoller.java     # 100ms 주기 SKIP LOCKED 폴링
...
```json
{
  "messageId": 15, "channelId": 1,
  "senderId": "uuid", "senderName": "이름",
  "content": "안녕", "seq": 15, "createdAt": 1234567890
}
```
