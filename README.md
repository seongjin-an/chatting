...
- 메시지 흐름 — WebSocket 수신부터 DB 저장, 다시 브라우저 전달까지 9단계
- 모듈 구성표 — 포트, 역할 한눈에
...
│ - 로그인   │                │ - 멤버 관리   │  │ - 세션 레지스트리  │
│ - JWT 발급 │                │ - 메시지 저장 │  │ - Kafka 발행       │
│ - 토큰갱신 │                │ - 메시지 조회 │  │ - Kafka 소비       │
│ - Redis    │                │ - Kafka 소비  │  │ - Redis 연결정보   │
│   블랙리스트│                │ - Redis 라우팅│  │   저장/조회        │
└────────────┘                └──────┬────────┘  └────────┬───────────┘
                                     │                    │
                         ┌───────────┴──┐    ┌───────────┴──┐
                         │    Kafka     │    │    Redis      │
...
message-service  (Kafka consumer)
  │  3. Redis INCR → seq 생성 (channel:seq:{channelId})
  │  4. MySQL 저장 (PK: channelId + seq 복합키)
  │  5. Redis 조회 → 채널 멤버의 WebSocket 연결 인스턴스 확인
  │     ws:user:{userId}         → connectionKey Set
  │     ws:connection:{key}      → ConnectionInfo JSON (instanceId 포함)
  │  6. Kafka 발행 → [connection-instance-{instanceId}] topic (인스턴스별)
  ▼
connection-service  (Kafka consumer, 자신의 instanceId 토픽만 소비)
  │  7. WebSocket 세션 레지스트리에서 userId로 세션 조회
  │  8. TextMessage 전송 (ContentMessage JSON)
  ▼
Browser
  9. 메시지 수신 → Zustand 스토어 업데이트 → UI 렌더링
```
...
| `message-service` | 8083 | 채널/메시지 CRUD, Kafka 소비·발행 |
...
| `channel:seq:{channelId}` | String (INCR) | 채널별 메시지 seq 생성 |
...
| `connection-instance-{instanceId}` | message-service | connection-service (인스턴스별) | 처리 완료 메시지 → WS 전달 |

- 파티션: 3, 레플리카: 1 (개발 환경)
- 파티션 키: `{channelId}-{senderId}` → 동일 채널·발신자 메시지 순서 보장
...
> `message` 테이블의 PK를 `(channel_id, seq)` 복합키로 설계하여  
> `WHERE channel_id = ? ORDER BY seq` 쿼리가 인덱스 스캔만으로 처리됨
...
서비스 이름: `eureka-server` · `api-gateway` · `user-service` · `connection-service` · `message-service` · `frontend`
...
| `KAFKA_BOOTSTRAP_SERVERS` | `localhost:9092` | connection, message |
